...
//--------------------------------------------------------
//
//Integer, Real, UInteger and UReal operations.
//
//--------------------------------------------------------
...
final class Op_number_sqrt extends OpGeneric {
	...
	public Type matches(Type params[]) {
		if (params.length == 1) {
			if (params[0].isTypeOfUInteger())
				return TypeFactory.mkUInteger();
			else if (params[0].isTypeOfUReal())
				return TypeFactory.mkUReal();
			else if (params[0].isKindOfNumber(VoidHandling.EXCLUDE_VOID))
				return TypeFactory.mkReal();
		}
	...
	public Value eval(EvalContext ctx, Value[] args, Type resultType) {
		if (args[0].isUInteger()) {
			return ((UIntegerValue) args[0]).sqrt();
		}
		else if (args[0].isUReal()) {
			return ((URealValue) args[0]).sqrt();
		}
		else {
			double d1;
			if (args[0].isInteger())
